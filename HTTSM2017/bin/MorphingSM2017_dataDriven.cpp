...
int main(int argc, char** argv) {
    ...
    ("shape_systematics", po::value<bool>(&do_shape_systematics)->default_value(true))
...
// VString chns = {"et"};
    ...
    if (do_jetfakes){
      ...
      //bkg_procs["mt"] = {"ZL", "TTT", "VV", "EWKZ", "JetFakes"}; // Doyeong
      bkg_procs["mt"] = {"ZL", "TTT", "VVT", "EWKZ", "jetFakes"}; // Tyler
      bkg_procs["tt"] = {"ZL", "TTT", "VVT", "EWKZ", "JetFakes"};
    }else{
    ...
    if (do_embedded){
      //bkg_procs["et"].push_back("embed");
      bkg_procs["et"].push_back("embedded");
      //bkg_procs["mt"].push_back("embedded"); // Doyeong
//      bkg_procs["mt"].push_back("embed"); // Tyler
      bkg_procs["mt"].push_back("embedded"); // Tyler
...
//    vector<string> sig_procs = {"ggH","VBF","ZH"};
...
//    cb.AddProcesses(   {"*"}, {"htt"}, {"13TeV"}, {"et"}, {"EWKZ"}, {{1, "et_0jet"},{2, "et_boosted"},{3, "et_vbf"}}, false);
    ...
    ch::AddSMRun2Systematics_dataDriven(cb, control_region, mm_fit, ttbar_fit, do_shape_systematics);
